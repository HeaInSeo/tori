...
- [ ] 현재 패키지들을 위 관점으로 임시 분류한다.
- [ ] core가 transport/runtime에 오염되는 지점을 찾는다.
- [ ] Track A 이전에 경계 잠금이 필요한 최소 지점을 제안한다.
...
- [ ] 현재 rule resolver 관련 코드 위치 확인
- [ ] fixture 추가 위치 확인
- [ ] golden/snapshot test 추가 가능성 점검
- [ ] 문서와 실제 코드 경로의 불일치 확인
- [ ] 현재 pair-end 예시 rule이 어디까지 구현돼 있는지 재정리
...
아래는 Track A 시작 전에 정리해야 할 가능성이 높은 항목이다.

- `go test ./...` 는 보류 영역(`service`)로 인해 여전히 실패 상태
- README 온보딩 기준 부재
- 패키지 경계 후보 문서화 미완료
...
- `go test` 상태가 이해되고 분류되었다.
- 최소 Green baseline 전략이 정의되었다.
- lint/toolchain baseline이 도입되었다.
- 개발자 진입 명령이 문서화되었다.
- README 최소 온보딩 기준이 반영되었다.
- 패키지 경계 후보가 정리되었다.
- Track A를 시작할 수 있는 첫 개발단위가 제안되었다.
...
지금 tori는 Track A 개발을 시작할 수 있는 설계 기준선은 갖추고 있으나, 그것을 지속적으로 검증할 최소 개발환경 baseline은 아직 약하다.

따라서 지금 단계의 최우선 목표는 기능 확장이 아니라, **Track A 개발을 안전하게 받을 수 있는 Phase 0 baseline 확보**다.
