...
A-1 historical/current anchor에서는 마지막 값 overwrite가 관찰되었다.
현재 활성 v0.1 구현 contract는 `DuplicateCollisionError` 반환이며, 이 문서는 그 최소 기준선을 정리한다.
...
1. `ReasonCode` (예: `duplicate_role_in_row`)
2. `RowKey`
3. `RoleKey` (현재 column key)
4. `Candidates` (충돌한 후보 파일명 배열)
5. `SourceFileNames` (입력 파일명 컨텍스트; v0.1에서는 `Candidates`와 동일 허용)
6. `Diagnostic` (optional text, 비어 있어도 됨)

권장 규칙:
- `Candidates`는 deterministic ordering(예: sort)으로 기록
- `SourceFileNames`도 현재 구현에서는 deterministic ordering으로 기록된다.
- `Diagnostic`은 정책 설명이 아니라 관찰 사실만 기록한다.

v0.1 non-goals / not guaranteed:
- `Entries` 순서는 v0.1 contract에서 보장하지 않는다.
- `Candidates`의 원래 입력 순서는 v0.1 contract에서 보장하지 않는다.
- `SourceFileNames`의 원래 입력 순서도 v0.1 contract에서 보장하지 않는다.
- `Diagnostic`은 best-effort 필드이며 비어 있을 수 있다.
- multi-role 일반화는 v0.1 범위 밖이며, 현재 v0.1은 최소 contract만 규정한다.
...
- `Entries` 길이, `ReasonCode`, `RowKey`, `RoleKey`, `Candidates`, `SourceFileNames`가 기대값과 일치해야 한다.
