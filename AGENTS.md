...
4. **Phase A-2 Duplicate Policy Design** (현재 진행 주제)
5. **Phase A-1 Current Semantics Freeze 결과를 regression anchor로 유지**
...
2. `docs/fileblock_rule_resolution_spec_v0.1.1.md`
3. `docs/phase_a1_current_semantics_freeze_workplan_v0.1.1.md`
4. `docs/tori_phase0_environment_setup_checklist_v0.1.md`
5. `docs/duplicate_policy_design_note_v0.1.md`
...
- 현재 원격 테스트 장비는 `100.123.80.48` 이다.
- SSH 계정은 `seoy` 이다. 즉 접속 대상은 `seoy@100.123.80.48` 이다.
- `heain@100.123.80.48` 로 접속하지 않는다.
- 원격 테스트가 요구되면 이 장비에서 `make test-core` 와 `make test-shared-fs-fixtures` 를 우선 실행한다.
